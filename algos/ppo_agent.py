...
class PPOAgent(BaseAgent):
    ...
    def update_policy(self):
        ...
        self.action_log_probs = torch.stack(self.action_log_probs, dim=0).to(
            self.device).squeeze()  # if Normal:(bs, act_dim) elif Multivariate Normal: (bs, )
        # print(f'{self.action_log_probs.shape=}')
    ...
    def get_action(self, observation, evaluation=False):
        ...
        action = torch.clamp(input=action, min=-50.0,
                             max=50.0)  # change the min/max to +-45 (bc of the size of the robot itself)
        ...
        for ep in range(self.cfg.train_episodes + 1):
            ...
            if total_step % self.cfg.log_interval == 0:
                ...
                if not self.cfg.silent:
                    print(
                        f"Episode {ep} Step {total_step} finished. Average episode return: {average_return} ({train_info['episode_length']} episode_length, {logstd} logstd)")
