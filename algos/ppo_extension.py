...
class PPOExtension(PPOAgent):
    ...
    def ppo_update(self, states, actions, rewards, next_states, dones, action_log_probs, return_estimates):
        ...
        entropy = new_policy_action_dists.entropy().mean()  #IMPROVEMENT : entropy
        
        loss = 1.0*policy_objective + 0.5*value_loss - self.entropy_coef*entropy   #IMPROVEMENT : entropy with possibility of decaying its coeff
...
#         return torch.Tensor(list(reversed(TDn_return_estimate))) # (bs,)
    ...
    def get_action(self, observation, evaluation=False):
        ...
        if evaluation:
            observation = observation/50.0 #IMPROVEMENT for baseline : input normalization
            observation[observation==-1.4] = 0.0 #IMPROVEMENT : replaceing outlier -1.4 in observation space with 0.0
        ...
        action = torch.clamp( input=action,  #IMPROVEMENT : clampping action
    ...
    def train_iteration(self,ratio_of_episodes):
        ...
        while not done and episode_length < self.cfg.max_episode_steps:
            ...
            if len(self.states) > self.cfg.min_update_samples:
                ...
                # self.policy.set_logstd_ratio(ratio_of_episodes)   #IMPROVEMENT : decaying the logstd or std of the action distribution
                # self.entropy_coef = 0.01 * ratio_of_episodes**6    #IMPROVEMENT : entropy with possibility of decaying its coeff
